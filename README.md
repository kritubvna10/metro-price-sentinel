...
### Deploy to Netlify (current live site)

```bash
# Option A: Drag-and-drop
# Go to netlify.com → "Add new site" → "Deploy manually" → drag the folder

# Option B: Link GitHub repo
# Netlify → "Add new site" → "Import from Git" → select repo
# Build command: (leave blank)
# Publish directory: . (or the subfolder containing index.html)
```
...
> **Note:** Some browsers block `fetch()` on `file://` URLs. If the dashboard shows a CSV load error when opened directly, use the Python server above or deploy to GitHub Pages / Netlify.
